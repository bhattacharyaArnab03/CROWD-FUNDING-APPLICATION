...
async function connectRabbitMQWithRetry() {
    while (true) {
        try {
            ...
            channel.consume(USER_REGISTERED_QUEUE, async (msg) => {
                if (msg !== null) {
                    try {
                        ...
                        console.log("[Campaign Service][RabbitMQ] Consumed user.registered event:", eventData);
                        channel.ack(msg);
                    } catch (e) {
            ...
            connection.on("close", () => {
                console.warn("[Campaign Service][RabbitMQ] Connection closed. Attempting to reconnect...");
                reconnectRabbitMQ();
            });
...
async function reconnectRabbitMQ() {
    ...
    while (!reconnected) {
        try {
            ...
            console.log("[Campaign Service][RabbitMQ] Reconnected successfully");
            reconnected = true;
        } catch {
            await new Promise((res) => setTimeout(res, 5000));
        }
